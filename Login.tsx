...
import { Linking, ScrollView, Button, TouchableOpacity, TextInput, View, Image, Text, ViewStyle, Switch, TextStyle, ActivityIndicator, ImageStyle, SafeAreaView } from "react-native"
      ...
      fetch(websiteurl.value + '/wp-json/jwt-auth/v1/token', requestOptions)
        .then(async response => {
          ...
          if (!response.ok) {
            ...
            let error = 'Invalid Website URL or Login. Recheck and login.';
            //if (data && data.message) {
            //const regex = /(<([^>]+)>)/ig;
            //error = data.message.replace(regex, '');
            // response.status
            setErrortext(error);
          ...
          const storeUserData = async () => {
            //console.log(data);
            if (websiteurl.value == '') {
              await saveString("user_websiteurl", 'https://staging.bucklit.com.au/service-tracking/wordpress/');
            }
            else {
      ...
      >
        <View style={globalstyle.FULL}>
          ...
          <View style={globalstyle.FOOTER_CONTENT}>
            ...
            <TouchableOpacity style={globalstyle.clickhere} onPress={() => Linking.openURL('mailto:info@bucklit.com.au?subject=Service Tracker Mobile Application&body=Description')}>
              <Text style={globalstyle.TEXT}>
                Please let me know if you have any questions or concerns. Click 'here' to email us or contact us at info@bucklit.com.au.
              </Text>
            </TouchableOpacity>
          </View>
